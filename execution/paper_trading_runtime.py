...
class PaperTradingRuntime:
    ...
    def __init__(self, session, cycle=None):
        self.session = session
        self.cycle = cycle or PaperTradingCycle(session)
    ...
    def run(self, cycles, symbol="BTCUSDT"):
        ...
        for cycle_input in cycles:
            result = self.cycle.process(
                ...
                symbol=cycle_input.get("symbol", symbol),
    ...
    def build_summary(self, results):
        open_count = sum(result["action"] == "OPEN" for result in results)
        close_count = sum(result["action"] == "CLOSE" for result in results)
        hold_count = sum(result["action"] == "HOLD" for result in results)
